 ...
 */
    ...
    function calcularImposto() {
        ...
        if (Number(valorProduto) < 50) {
            resultadoImposto = Number(valorProduto) + (Number(valorProduto) * 0.20);
        } else {
            resultadoImposto = (Number(valorProduto) + (Number(valorProduto) * 0.20)) * 1.90;
        }

        document.getElementById("resultado-imposto").innerText = resultadoImposto;

        salvarHistoricoImposto(valorProduto, resultadoImposto);

    }

    function salvarHistoricoImposto(valorProduto, resultadoImposto) {
        console.log("Valor do produto: " + valorProduto);
        console.log("Resultado do imposto: " + resultadoImposto);
        console.log("--------------------------------------");

        let imposto = {
            valorProduto: valorProduto,
            resultadoImposto: resultadoImposto
        };

        let historicoImpostoLocal = JSON.parse(localStorage.getItem("historicoImpostos")) || [];
